...
4. O playbook executa em paralelo nas máquinas `mach1`, `mach2` e `mach3`:
   ...
   - `pm2 start ... --only <process>` para garantir web (cluster) e workers (fork).
...
- Primário PostgreSQL em `mach3` com réplicas sincronizadas (`pg_is_in_recovery() = true`).
...
5. `pm2 start ... --only late-prod` garante instâncias web.
6. `pm2 start ... --only late-prod-email-worker/late-prod-export-worker` garante workers.
...
- **PM2 não sobe em cluster**: verifique `ecosystem.config.js` (web `instances: 'max'`, workers `fork`).
...
- Adicionar health-check pós-playbook (curl `/api/health`, `/relatorios/status`).
