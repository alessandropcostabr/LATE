...
-- Sprint 1: Migração SQL para PostgreSQL com identidade e timestamps padrão.
...
-- Usamos GENERATED AS IDENTITY para garantir auto-incremento.
...
-- Campos funcionais mantidos como TEXT para simplificar validação na aplicação
  call_date TEXT,
  call_time TEXT,
  recipient TEXT,
  sender_name TEXT,
...
-- Validação de status garantindo domínio permitido
...
-- 2) CURRENT_TIMESTAMP mantém consistência sem depender de funções específicas.
-- 3) O CHECK de 'status' garante que apenas valores conhecidos sejam persistidos.
