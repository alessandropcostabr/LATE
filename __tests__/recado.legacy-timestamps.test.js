...
process.env.DB_PATH = process.env.DB_PATH ?? ''; // força :memory: no SQLite pelo DB Manager

const dbManager = require('../config/database');
const MessageModel = require('../models/message');

function getCreateTableSQL(driver) {
  if (driver === 'pg') {
    return `
...
async function resetDatabase() {
  ...
  const driver = dbManager.adapter().name; // 'pg' | 'sqlite'

  await db.exec(`DROP TABLE IF EXISTS messages;`);
  await db.exec(getCreateTableSQL(driver));
  return db;
}
